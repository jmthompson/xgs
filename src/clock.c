...
int clockInit()
{
    ...
    if (fd < 0) {
        if (errno != ENOENT) {
            perror("Error loading BRAM save file");
        }
    }
    else {
...
void clockShutdown()
{
    ...
    if (fd < 0) {
        perror("Error writing BRAM save file");
