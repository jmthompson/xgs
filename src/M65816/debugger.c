...
void m65816_debug(void) {
	...
	printf("%02X/%04X    %s  ",(int) PC.B.B,(int) PC.W.L,mnemonics[opcode]);
	switch (mode) {
		...
		case 0x0E:	operand = M_READ(PC.A+1) | (M_READ(PC.A+2) << 8) | (M_READ(PC.A+3) << 16);
				...
				break;
		case 0x0F:	operand = PC.W.L + 2 + (offset_s) M_READ(PC.A+1);
				printf("$%04X       ", operand & 0xFFFF);
				break;
		case 0x10:	operand = PC.W.L + 3 + (offset_l) (M_READ(PC.A+1) | (M_READ(PC.A+2) << 8));
				printf("$%04X       ", operand & 0xFFFF);
				break;
