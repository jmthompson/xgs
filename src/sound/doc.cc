...
/*
 * This code handles emulation of the Ensoniq 5503 DO
 */
...
static unsigned int acc_masks[8] = {0x00FF, 0x01FF, 0x03FF, 0x07FF,
                                    0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF};

static unsigned int kSampleRates[kNumOscillators] = {
...
static void pushInterrupt(const unsigned int osc_num)
{
  if (irq_index == kInterruptStackSize) return;

  if (!irq_index) raiseInterrupt(DOC_IRQ);

  irq_stack[irq_index++] = osc_num;
}

static const int pullInterrupt()
{
  if (!irq_index) return -1;

  int osc_num = irq_stack[--irq_index];
  if (!irq_index) lowerInterrupt(DOC_IRQ);

  return osc_num;
}

static void updateOscillator(const unsigned int osc_num)
{
  unsigned int ctrl;
  ...
  osc_freq[osc_num] =
      (doc_registers[0x20 + osc_num] << 8) | doc_registers[0x00 + osc_num];
  ...
  osc_addrbase[osc_num] =
      doc_ram + ((osc_wp[osc_num] & wp_masks[osc_ws[osc_num]]) << 8);
  osc_enable[osc_num] = !(ctrl & 0x01);
  if (osc_enable[osc_num] && !(osc_num & 0x01) && (osc_mode[osc_num] == 2)) {
...
static void enableOscillators(void)
{
  ...
  for (i = 0; i < num_osc; i++) {
    ...
    last_mode = mode;
    doc_registers[0xA0 + i] = (doc_registers[0xA0 + i] & 0xFE) + !osc_enable[i];
    updateOscillator(i);
  }
}

static void scanOscillators(AudioSample *samples_out)
{
  int addr;
  int osc_num;
  float sample;
  ...
  for (osc_num = 0; osc_num < num_osc; osc_num++) {
    ...
    if (addr < last_addr[osc_num]) {
      if (!osc_mode[osc_num]) {
        ...
        if (osc_int[osc_num]) pushInterrupt(osc_num);
